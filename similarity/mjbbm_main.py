...
df=pd.read_csv(r'E:\antiy\NOTE_NEW\similarity\hive_liuan_sample_same_mjbbm.csv')
max_row=df.shape[0]
step=50000
...
def sub_df_sim(df,start_index,end_index):
    ...
    for idx,item in sub_df.iterrows():
        sent_1=item['hive_liuan_sample_same_mjbbm.program_name']
        sent_2=item['hive_liuan_sample_same_mjbbm.m_program_name']
        try:
...
for t in threads:
    t.join()
df.to_excel(r'E:\antiy\NOTE_NEW\similarity\mjbbm_sim.xlsx')
print(time.time()-t1)